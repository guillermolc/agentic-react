...
import { getCopilotPAT } from "../lib/providers.js";
import { kdbExternalRouter } from "./kdb-external.js";

export const kdbRouter = Router();

// Mount external KDB sub-router
kdbRouter.use("/external", kdbExternalRouter);
...
// GET /api/kdb/spaces
kdbRouter.get("/spaces", async (_req: Request, res: Response) => {
  const pat = getCopilotPAT();

  if (!pat) {
    res.status(503).json({ error: "Copilot provider not configured — set GITHUB_PAT in backend .env" });
    return;
  }
